...
//!
//! Reads the sidecar, runs ffprobe + frame extraction + LLM scoring on
//! the new clip, then writes the sidecar back with the new verdict
//! appended. Does not re-score existing clips.

use crate::analyzer::claude_print::ClaudePrintAnalyzer;
use crate::analyzer::ClipAnalyzer;
use crate::paths;
use crate::pipeline::{discover, frames};
use crate::preflight;
use crate::sidecar;
...
/// Run the add command.
pub(crate) async fn run(input_dir: &Path, clip_path: &Path, out: Option<PathBuf>) -> Result<()> {
    ...
    let clip = discover::describe(&clip_path)
        .await
